-- Corrigir o erro de ambiguidade na função generate_weekly_analysis
-- Remover triggers existentes e função com CASCADE
DROP TRIGGER IF EXISTS trigger_weekly_analysis ON weight_measurements;
DROP TRIGGER IF EXISTS trigger_generate_weekly_analysis ON weight_measurements;
DROP FUNCTION IF EXISTS generate_weekly_analysis() CASCADE;

-- Recriar a função corrigida sem ambiguidade
...
BEGIN
  ...
  -- Buscar primeira pesagem da semana com alias para evitar ambiguidade
